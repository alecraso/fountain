...
- `sandbox_parking` joins `provisioning`, `sprite_probe_failed` and `fleet_full` in the codes that map to `.notReady` in both products: the reaper holds a durable park claim on the sandbox mid-checkpoint or mid-provider-suspend-call, and the same call will work once it clears (#2291).
