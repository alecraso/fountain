defmodule Fountain.Conversations.Interruption do
  ...
  session if one exists. If the sandbox is dead or stranded — gone, never
  provisioned, or stuck `pending`/`starting` with no server ever turning up —
  ...
  place instead, fenced to the sandbox actually probed
  (`Wake.reconcile_dead_interrupt/1`, decided 2026-09-15, #2175 open decision
  1) and answers `:not_running`. Only a row that says `running` is worth a
  wake — an idle, terminated or unknown conversation has nothing running
  regardless, and must not pay for one it does not need.
