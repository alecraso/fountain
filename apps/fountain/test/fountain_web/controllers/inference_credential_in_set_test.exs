defmodule FountainWeb.InferenceCredentialInSetTest do
  ...
  test "malformed IDs refuse both operations before provider validation", %{conn: conn, key: key} do
    reject(Req, :get, 2)
    path = credential_path(%{id: "nope"}, "anthropic_api_key")

    assert conn
           |> authed_with_key(key)
           |> put_json(path, %{"value" => "sk-key"})
           |> json_response(404)

    assert conn |> authed_with_key(key) |> delete(path) |> json_response(404)
  end

  test "a sprite key cannot clear a selected set", %{user: user, second: second} do
    {:ok, dek} = Crypto.load_tenant_key(user.id)
    {:ok, _} = InferenceCredentials.put_credential_in(second, dek, :anthropic_api_key, "keep")
    {_, key} = insert_sprite_api_key(user)
    ...
    assert value_in(second, :anthropic_api_key) == "keep"
  end
